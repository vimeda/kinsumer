...
func (k *Kinsumer) getShardIterator(shardID, sequenceNumber string) (string, error) {
...
func (k *Kinsumer) getRecords(iterator string) (records []*kinesis.Record, nextIterator string, lag time.Duration, err error) {
	params := &kinesis.GetRecordsInput{
		Limit:         aws.Int64(getRecordsLimit),
		ShardIterator: aws.String(iterator),
	}

	output, err := k.kinesis.GetRecords(params)
	if err != nil {
		return nil, "", 0, err
	}
...
func (k *Kinsumer) consumePolling(ctx context.Context, shardID string, sequenceNumber string) <-chan consumeEvent {
	...
	go func() {
		...
		for {
			...
			iterator, err := k.getShardIterator(shardID, sequenceNumber)
			if err != nil {
		...
		getrecordloop:
			for {
				...
				records, next, lag, err := k.getRecords(iterator)
				if err != nil {
